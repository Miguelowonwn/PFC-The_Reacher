...
func _get_all_scores() -> Array:      
	return database.select_rows("Scores", "" , ["usuario_id, nivel_id, puntuacion, tiempo_completitud"])     
...
func _mostrar_lista(scores: Array) -> void:
	...
	for info in filtrados:
		id_usuario = _get_name_from_id(usuarios_id_nombre, info.usuario_id)
		nivel_id = info["nivel_id"]
		puntuacion = info["puntuacion"]
		tiempo = info["tiempo_completitud"] 
		var texto = "Nivel: %-6s Usuario: %-12s Puntuación: %-8s Tiempo: %s" % [str(nivel_id),str(id_usuario),str(puntuacion),str(tiempo)]
		add_item(texto)
